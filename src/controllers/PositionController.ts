...
class PositionController {
  ...
  static createPosition = async (req: Request, res: Response) => {
    const { name } = req.body;
    ...
    try {
      const pos_exist = await model.findFirst({
        where: {
          name: {
            contains: name,
          },
      ...
      if (pos_exist) {
        return res.status(400).send("Position Already Existed");
      }
  ...
  static updatePosition = async (req: Request, res: Response) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    }
    try {
      ...
      const position = await model.update({
        where: { id: Number(id) },
        data: {
          name,
        },
